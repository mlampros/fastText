...
void FastText::printInfo(real progress, real loss, std::ostream& log_stream, std::ofstream& out, std::string pth) {
  ...
  if (pth != "") {
    ...
    stream << "Progress: " << std::setprecision(1) << std::setw(5) << progress << "%" << " words/sec/thread: " << std::setw(7) << int64_t(wst) << " lr: " << std::setw(9) << std::setprecision(6) << lr << " loss: " << std::setw(9) << std::setprecision(6) << loss << " ETA: " << std::setw(3) << etah << "h" << std::setw(2) << etam << "m\n";
    out << stream.str();
  }
...
void FastText::quantize(const Args& qargs) {     // reduce the MilliSecs to increase the verbosity if the 'pth' parameter is not ""
  if (args_->model != model_name::sup) {
    throw std::invalid_argument(
  ...
  if (qargs.cutoff > 0 && qargs.cutoff < input_->size(0)) {
    ...
    if (qargs.retrain) {
      ...
      startThreads("", 100);                          // in case of the 'quantize' function use the default parameters (in first place)
    }
  }
...
// // depreacted, use getAnalogies instead                  // I commented (didn't modified) this function because it's deprecated and not used elsewhere
...
void FastText::train(const Args& args, std::string pth = "", int MilliSecs = 100) {     // reduce the MilliSecs to increase the verbosity if the 'pth' parameter is not ""
...
void FastText::startThreads(std::string pth, int MilliSecs) {
  ...
  if (pth != "") {
    ...
    if (file_exists) {
      if ( remove( pth.c_str() ) != 0 ) {
        Rcpp::Rcout << "Error deleting the Progress-log-file !" << std::endl;
      }
    }
  ...
  while (tokenCount_ < args_->epoch * ntokens) {
    std::this_thread::sleep_for(std::chrono::milliseconds(MilliSecs));      // 100
    if (loss_ >= 0 && args_->verbose > 1) {
      real progress = real(tokenCount_) / (args_->epoch * ntokens);
      Rcpp::Rcout << "\r";
      printInfo(progress, loss_, Rcpp::Rcout, out, pth);
    }
  }
  ...
  if (args_->verbose > 0) {
    Rcpp::Rcout << "\r";
    printInfo(1.0, loss_, Rcpp::Rcout, out, pth);
    Rcpp::Rcout << std::endl;
  }
}
