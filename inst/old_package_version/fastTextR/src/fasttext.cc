...
void FastText::saveVectors() {
  ...
  // ofs << dict_->nwords() << " " << args_->dim << std::endl;             // exclude dimensions from output .txt file
...
//   while (std::cin >> word) {
//     getVector(vec, word);
//     std::cout << word << " " << vec << std::endl;        // write to file
//   }
// }


void FastText::wordVectors(std::string& INPUT, std::string& OUTPUT) {
  
  std::ifstream inf(INPUT);
  
  std::ofstream ofs(OUTPUT + ".vec");
  if (!ofs.is_open()) {
    std::cout << "Error opening file for saving vectors." << std::endl;
    exit(EXIT_FAILURE);
  }
...
void FastText::textVectors() {
  ...
  while (std::cin.peek() != EOF) {
    ...
    std::cout << vec << std::endl;                    // write to file
  }
}

void FastText::printVectors() {
  if (args_->model == model_name::sup) {                    // if model 'supervised' return textvectors else return wordvectors
    textVectors();
  }
  // } else {
  //   wordVectors();
